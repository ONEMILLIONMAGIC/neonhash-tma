...
import { validateTgInitData, parseTgUser, upsertUser, calcMined, getLevel, setCors } from './_utils';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  return res.json({
    user: {
      ...
      total_mined: parseFloat(user.total_mined), upgrades_owned: user.upgrades_owned,
      referrals_count: 0,
    }
  });
}
